...
func NewJWT() *JWT {
	return &JWT{
		AccessToeknSecret:  []byte(global.Config.Jwt.AccessTokenSecret),
...
func (j *JWT) CreateAccessToken(claims request.JwtCustomClaims) (string, error) {
	...
	return token.SignedString(j.AccessToeknSecret)
}
...
func (j *JWT) ParseAccessToken(tokenString string) (*request.JwtCustomClaims, error) {
	claims, err := j.parseToken(tokenString, &request.JwtCustomClaims{}, j.AccessToeknSecret)
	if err != nil {
		return nil, err
	}
