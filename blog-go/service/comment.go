...
func (commentService *CommentService) CommentNew() (interface{}, error) {
	var comments []database.Comment
...
func (commentService *CommentService) CommentDelete(c *gin.Context, req request.CommentDelete) error {
	...
	return global.DB.Transaction(func(tx *gorm.DB) error {
		for _, id := range req.IDs {
			...
			if err := commentService.DeleteCommentChildren(tx, id); err != nil {
				return err
			}
		}
		return nil
...
func (commentService *CommentService) CommentList(req request.CommentList) ([]database.Comment, int64, error) {
	db := global.DB
	...
	if req.UserUUID != nil {
		db = db.Where("user_uuid = ?", *req.UserUUID)
	}
