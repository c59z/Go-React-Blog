...
func (configApi *ConfigApi) GetGithub(c *gin.Context) {
	response.OkWithData(global.Config.Github, c)
}

func (configApi *ConfigApi) UpdateGithub(c *gin.Context) {
	var req config.GitHub
	err := c.ShouldBindJSON(&req)
	if err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	...
	err = configService.UpdateGithub(req)
	if err != nil {
